...
def spawn_point():
    ...
    if default_scenario == 'cliff':
        ...
        if road_id == "9039": # good candidate for input rect.
            ...
            return {'pos': (290.558, -277.280, 46.0), 'rot': None, 'rot_quat': turn_X_degrees((0, 0, -0.277698, 0.960669), -115)}
        else:
...
def run_scenario(vehicle, bng, scenario, model, vehicle_model='etk800',
                 ...
                 device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'), vae=None):
    ...
    with open(f"{writedir}/data.txt", "w") as f:
        ...
        while overall_damage <= 0:
            ...
            #     rectimg = rectified_image[0].permute(0, 2, 3, 1).detach().cpu().numpy()
            #     cv2.imshow('rectified_img', rectimg[0])
            #     cv2.waitKey(1)
            #     print(f"old prediction={prediction}")
            #     prediction = model(rectified_image[0])
...
def main():
    ...
    vae_name = "vae/VAEsteer-4thattempt-nobatchnorm.pt"
    ...
    vehicle, bng, scenario = setup_beamng(vehicle_model='hopper', camera_pos=camera_pos, camera_direction=camera_dir)

    for i in range(1):
        results = run_scenario(vehicle, bng, scenario, model, vehicle_model='hopper', camera_pos=camera_pos, camera_direction=camera_dir, run_number=i, vae=vae)
