...
class CarEnv(gym.Env):
    ...
    def step(self, action):
        if self.test_model:
            ...
            while self.evaluator(outside_track, distance_from_center, leftrightcenter, segment_shape) == 1:
            # while False:
                done = outside_track or self.state["collision"]
                obs = self._get_obs()
                if done:
            ...
            expert_action, cartocl_theta_deg = self.get_expert_action(outside_track, distance_from_center, leftrightcenter, segment_shape, theta_deg)
            blackedout = np.ones(image.shape)
            blackedout[:,:,:2] = blackedout[:,:,:2] * 0 # RED
            cv2.imshow("action image", blackedout)
            cv2.waitKey(1)
            if abs(expert_action) > 0.2:
                self.setpoint = 30
            else:
                self.setpoint = 40
    ...
    def calc_reward(self, action, expert_action, evaluation):
        ...
        if round(conf, 0) == evaluation:
            ...
            reward -= 10
        print(f"{action=} \t conf={round(conf, 0)} \t {evaluation=} \t conf. reward={reward} \t action reward={-(abs(expert_action - action[0][1])*100)}")
