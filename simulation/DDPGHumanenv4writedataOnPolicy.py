...
class CarEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.test_model:
            ...
            # print(outside_track, self.state["collision"], (self.distance(self.state["pose"][:2], cutoff_point[:2]) < 12))
                        ...
                        self.distance(self.vehicle.state["pos"][:2], cutoff_point[:2]) < 12)
            ...
            position = str(self.vehicle.state["pos"]).replace(",", " ")
            ...
            self.episode_steps += 1
            done = outside_track or self.car_state["damage"]["damage"] > 1 or (self.distance(self.vehicle.state["pos"][:2], cutoff_point[:2]) < 12)
            self.current_rewards.append(reward)
    ...
    def calc_reward(self, agent_action, expert_action):
        if self.car_state["damage"]["damage"] > 1:
            return -1000
    ...
    def calc_reward_onpolicy(self, outside_track, distance_from_center):
        if outside_track or self.car_state["damage"]["damage"] > 1:
            return -1000
        else:
