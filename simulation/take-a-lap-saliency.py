...
def spawn_point():
    ...
    if default_scenario == 'cliff':
        ...
        #     return {'pos': (-401.98, 243.3, 25.5), 'rot': None, 'rot_quat': (0, 0, -0.277698, 0.960669)}
            ...
            return {'pos': (-315.2, 80.94, 32.33), 'rot': None,
                    'rot_quat': turn_X_degrees((0, 0, -0.2777, 0.9607), -20)}
        else:
...
def setup_beamng(vehicle_model='etk800'):
    ...
    # -6.55275,227.062,26.0217 0.00015590760449413,0.012093841098249,-0.68325901031494,0.73007589578629
    # -4.71639,226.368,26.0738 0.00031242781551555,0.011255933903158,-0.67924827337265,0.73382222652435
...
def run_scenario(vehicle, bng, scenario, model, vehicle_model='etk800', run_number=0,
    ...
    with open(f"{writedir}/data.txt", "w") as f:
        ...
        while overall_damage <= 0:
            ...
            #     s = 8
            #     p1 = 0.50
            #     masks = RISE.explanations.generate_masks(N, s, p1)
            #     masks = masks.squeeze(3)
            #     img = processed_img
            #     label = prediction.item()
            #     sal = RISE.explanations.explain(model, img, masks, N, p1)
