...
def setup_beamng(default_scenario, road_id, reverse=False, seg=1, img_dims=(240,135), fov=51, vehicle_model='etk800', default_color="green", steps_per_sec=30):
    ...
    beamng = BeamNGpy('localhost', 64956, home='C:/Users/Meriel/Documents/BeamNG.research.v1.7.0.1', user='C:/Users/Meriel/Documents/BeamNG.researchINSTANCE4')
...
def run_scenario(vehicle, bng, scenario, model, default_scenario, road_id, reverse=False, vehicle_model='etk800', run_number=0,
    ...
    spawn = spawn_point(default_scenario, road_id, reverse=reverse)
...
def main():
    ...
    # model_name = "../models/weights/dave2-weights/model-DAVE2v3-bigimg270x480-lr1e4-100epoch-batch64-lossMSE-82Ksamples-INDUSTRIALandHIROCHIandUTAH-135x240-noiseflipblur.pt"
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = torch.load(model_name, map_location=device).eval()

    img_dims = (120,67) # (120,67) # (240,135) # (480, 270)
    reverse = False
    default_scenario = 'west_coast_usa' # 'hirochi_raceway' #'west_coast_usa' 'automation_test_track' 'industrial'
    road_id = "10988" # "8185" # "9039" #"12930" # "10988"
    seg = 1
    fov = 51
    # training_file = 'metas/training_runs_{}-{}1-reverse={}.txt'.format(default_scenario, road_id, reverse)
    ...
    print(f"OUT OF 5 RUNS:\n\tAverage distance: {sum(distances)/len(distances)}"
          f"\n\tAverage deviation: {sum(deviations) / len(deviations)}"
          f"\n\t{distances=}"
          f"\n\t{deviations:}")
    bng.close()
