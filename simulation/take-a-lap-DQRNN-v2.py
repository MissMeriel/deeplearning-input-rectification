...
def run_scenario(vehicle, bng, scenario, vehicle_model='etk800', pitch_euler=0.0, ep=0,
    ...
    # last_screen = torch.zeros(1, 3, DQRNNv2.screen_height, DQRNNv2.screen_width)
    # current_screen = torch.zeros(1, 3, DQRNNv2.screen_height, DQRNNv2.screen_width)
    state = torch.zeros(1, 9, DQRNNv2.screen_height, DQRNNv2.screen_width)
    ...
    while overall_damage <= 0 or not outside_track or not done:
        ...
        action = DQRNNv2.select_action(torch.from_numpy(input).float(), total_loops)
        total_predictions += 1
        ...
        DQRNNv2.memory.push(state, action, next_state, reward)
        state = next_state

        # Perform one step of the optimization (on the policy network)
        DQRNNv2.optimize_model()
        if done:
            DQRNNv2.episode_durations.append(total_loops + 1)
            DQRNNv2.plot_durations(save=(ep % 50 == 0))
            break
...
def main():
    ...
    for i_episode in range(num_episodes):
        ...
        # Update the target network, copying all weights and biases in DQRNNv2
        if i_episode % DQRNNv2.TARGET_UPDATE == 0:
            DQRNNv2.target_net.load_state_dict(DQRNNv2.policy_net.state_dict())
            DQRNNv2.save_policies(i_episode)
