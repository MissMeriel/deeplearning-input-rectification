...
def setup_sensors(transf, obs_shape, vehicle):
    ...
    if transf == "depth":
        ...
        far_camera = Camera(camera_pos, camera_dir, fov, resolution, #(obs_shape[2], obs_shape[1]),
...
def plot_rewards_by_steps(all_rewards, all_step_counts, save=False, title="temp", savetitle="rewards_steps_training_performance"):
    plt.figure(4, figsize=(10, 10), dpi=100)
    plt.clf()
    rewards_t = torch.tensor(all_rewards, dtype=torch.float)
    # durations_t = torch.tensor(episode_durations, dtype=torch.float)
    plt.title('Training rewards by total samples')
    plt.xlabel('Total samples')
    plt.ylabel('Rewards')
    cumulative_steps = [sum(all_step_counts[:i]) for i in range(len(all_step_counts))]
    plt.plot(cumulative_steps, rewards_t.numpy(), label=savetitle)
    # plt.plot(durations_t.numpy(), "--")
    # Take 100 episode averages and plot them too
    if len(rewards_t) >= 20:
        means = rewards_t.unfold(0, 20, 1).mean(1).view(-1)
        means = torch.cat((torch.ones(19) * means[0], means))
        plt.plot(cumulative_steps, means.numpy(), '--')
    plt.legend()
    if save:
        plt.savefig(f"{title}/{savetitle}.jpg")
