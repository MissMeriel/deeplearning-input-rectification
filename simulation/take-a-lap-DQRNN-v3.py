...
def run_scenario(vehicle, bng, scenario, model=None,
    ...
    while overall_damage <= 0 or not outside_track or not done:
        ...
        if kph < 30:
            ...
            action_probs = model(torch.tensor(state.reshape((1, 3, 135, 240)), dtype=torch.float32, device=device))
    ...
    return np.vstack(states), np.vstack(actions), np.vstack(probs), np.vstack(rewards), total_loops, traj
...
def plot_durations(rewards, episode_durations, save=False, title="temp"):
    # plt.figure(2)
    plt.figure(4, figsize=(3,3), dpi=80)
    plt.clf()
...
def main():
    ...
    for episode in range(num_episodes):
        states, actions, probs, rewards, duration, traj = run_scenario(vehicle, bng, scenario, model=model)
        trajectories.append(traj)
        one_hot_actions = np.eye(n_actions)[actions.T][0]
        gradients = one_hot_actions - probs
        dr = discounted_rewards(rewards, gamma=0.9, normalize=True)
        gradients *= dr
        target = alpha * np.vstack([gradients]) + probs
        print(f"Episode {episode} {states.shape=}")
        print(f"Episode {episode} -> Reward:{np.sum(rewards):.2f}\tmean target={np.mean(target)}\tmedian target={np.median(target)}")
        dataset = torch.utils.data.TensorDataset(torch.from_numpy(states), torch.from_numpy(target))
        train_one_epoch(model, dataset, episode, device=device)
        history.append(np.sum(rewards))
        durations.append(duration)
        plot_durations(history, durations, save=(episode % 10 == 0))
        torch.save(model.state_dict(), PATH)
        plot_deviation(trajectories, "ResNet50", "Policy Gradient Learner")
