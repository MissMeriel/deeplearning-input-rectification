...
def run_scenario(vehicle, bng, scenario, model=None,
    ...
    while overall_damage <= 0 or not outside_track or not done:
        ...
        if kph < 30:
            ...
            action, est_rew = model(torch.tensor(state.reshape((1, 3, 135, 240)), dtype=torch.float32, device=device))
            action = action.cpu().detach().numpy()[0,0]
            est_rew = est_rew.cpu().detach().numpy()
            print(f"cycle={total_loops:04}\t{distance_from_center=:.1f}\t{action=}\test_rew={est_rew.item()}")
        ...
        if kph > 30:
            ...
            states.append(state.reshape(3, 135, 240))
    ...
    return states, actions, rewards, critic_values, total_loops, traj
...
def plot_durations(rewards, episode_durations, save=False, title="temp"):
    ...
    if save:
        plt.savefig(f"training_performance-{title}.jpg")


def train_actor_critic(model, states, actions, critic_values, dr, device=torch.device("cpu")):
    loss_fn = nn.HuberLoss()
    states = torch.from_numpy(states)
    dr = torch.from_numpy(dr)
    training_set = torch.utils.data.TensorDataset(states, dr)
...
def main():
    ...
    # model = ResNet152(n_actions, channels=2).to(device)
    model = DQN(screen_height, screen_width, n_actions).to(device)
    newdir = ""
    PATH = "DQRNN-v5-actorcritic-continuousaction"
    max_reward = 0.0
    running_rewards = []
    try:
    ...
    for episode in range(num_episodes):
        states, actions, rewards, critic_values, duration, traj = run_scenario(vehicle, bng, scenario, model=model)
        trajectories.append(traj)
        episode_reward = np.sum(rewards)
        running_reward = 0.05 * episode_reward + (1 - 0.05) * running_reward
        # dr = discounted_rewards(rewards)
        print(f"Episode {episode} -> Episode Reward:{episode_reward:.2f}\tRunning reward:{running_reward:.2f}\tTraining time:{(time.time() - start_time)/60.0:.1f}min.")
        print(f"{np.array(states).shape=}")
        ...
        if episode % 10 == 0:
            ...
            plot_deviation(trajectories, "DQN", f"Actor-Critic cont-action {episode=}")
        if np.sum(rewards) > max_reward:
            torch.save(model.state_dict(), f"{PATH}.pt")
            max_reward = np.sum(rewards)

        if running_reward > 300000:  # Condition to consider the task solved
