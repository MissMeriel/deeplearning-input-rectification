...
def main():
    ...
    newdir = f"deletelater-supervisedcontrol1-corrpolicy-{timestr}-{randstr}"
    if not os.path.exists(newdir):
        os.mkdir(newdir)
        shutil.copy(f"{__file__}", newdir)
        shutil.copy(f"{os.getcwd()}/beamng_env.py", newdir)
    PATH = f"{newdir}/corrpolicy-"
    ...
    env = FaultInducingEnv(image_shape=(3, 135, 240), filepathroot=PATH, beamngpath='C:/Users/Meriel/Documents', beamnginstance="BeamNG.tech",
                 port=64156, road_id="15425", reverse=False, newdir=newdir, expert=orig_expert)
    ...
    model = DQN(
        ...
        device="cuda",
        tensorboard_log=f"./{newdir}/tb_logs_policymonitor1/",
    )
    ...
    # Save policy weights
    # corr_model.save(f"{PATH}-corr_model.pt")
