...
def spawn_point():
    ...
    if default_scenario == 'cliff':
        ...
        elif road_id == "7991": # long road, spawns at pitstop
        ...
        #     return {'pos': (-401.98, 243.3, 25.5), 'rot': None, 'rot_quat': (0, 0, -0.277698, 0.960669)}
        ...
        elif road_id == "9167": # veers away onto dirt road
            ...
            return {'pos': (-82.72785186767578, 10.69620418548584, 25.0), 'rot': None, 'rot_quat': turn_X_degrees((0, 0, -0.2777, 0.9607), 110)} # end
        else:
...
def run_scenario(vehicle, bng, scenario, model, vehicle_model='etk800', run_number=0,
    ...
    writedir = f"../CrashNet-traces/{default_scenario}-{road_id}-lap-CrashNet"
    ...
    with open(f"{writedir}/data.txt", "w") as f:
        ...
        past_dists = np.zeros((3 * steps_per_sec))
        past_imgs = np.zeros((3*steps_per_sec, 135, 240, 3), dtype=np.uint8)
        while overall_damage <= 0:
            ...
            if runtime > 3.0:
                ...
                pil_image.save(f"{writedir}/sample-{total_imgs-5*steps_per_sec:05d}.jpg", "JPEG")
                ...
                f.write(f"sample-{total_imgs:05d}.jpg,{prediction.item()},{position},{orientation},{kph},{sensors['electrics']['steering']},"
                        f"{past_dists[0]},{past_dists[2]},{past_dists[4]},{past_dists[steps_per_sec-1]},{past_dists[3*steps_per_sec-1]}\n")
            if abs(steering) > 0.2:
                setpoint = 30
            else:
                setpoint = 40
