...
def run_RLtrain(obs_shape=(3, 135, 240), scenario="hirochi_raceway", road_id="9039", seg=None, label="Rturn", transf="fisheye",
    ...
    newdir = f"F:/RRL-results/RLtrain-SB31.6.2-{policytype}-{noise_sigma}-onpolicy-1kpunish-{label}-{transf}-max{maxepisodes}-{eval_eps}eval-{timestr}-{randstr}"
    ...
    newdir_eval = f"F:/RRL-results/RLtrain-SB31.6.2-{policytype}-{noise_sigma}-onpolicy-1kpunish-{label}-{transf}-max{maxepisodes}-{eval_eps}eval-EVAL-{timestr}-{randstr}"
