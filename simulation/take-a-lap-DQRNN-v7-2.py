...
def setup_sensors(vehicle, image_size=(3, 135, 240)):
    camera_pos = (-0.5, 0.38, 1.3)
    camera_dir = (0, 1.0, 0)
    fov = 51 # 60 works for full lap #63 breaks on hairpin turn
    resolution = (image_size[2], image_size[1]) #(240, 135) #(400,225) #(320, 180) #(1280,960) #(512, 512)
...
def setup_beamng(vehicle_model='etk800', image_size=(3, 135, 240), camera_pos=(-0.5, 0.38, 1.3), camera_direction=(0, 1.0, 0), pitch_euler=0.0):
    ...
    vehicle = setup_sensors(vehicle, image_size=image_size)
    spawn = spawn_point()
...
def run_scenario(vehicle, bng, scenario, model=None, image_size=(3,135,240),
    ...
    while overall_damage <= 0 or not outside_track or not done:
        ...
        if kph < 30:
            ...
            action = output[0,0]
            est_rew = output[0,1]
            print(f"cycle={total_loops:04}\t{reward=}\t{distance_from_center=:.1f}\t{action=}\test_rew={est_rew}")
