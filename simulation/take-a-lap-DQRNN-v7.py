...
Black and white images with ResNet50
'''
...
def spawn_point():
    ...
    if default_scenario == 'cliff':
        ...
        if road_id == "9039": #"9039": # good candidate for input rect.
...
def plot_deviation(trajectories, model, deflation_pattern):
    ...
    plt.ylim([-300,-100])
    plt.draw()
    plt.savefig(f"{deflation_pattern}-{model}-trajs-so-far.jpg")
    plt.clf()
...
def run_scenario(vehicle, bng, scenario, model=None,
    ...
    while overall_damage <= 0 or not outside_track or not done:
        ...
        if kph < 30:
            ...
            output = model(torch.tensor(state.reshape((1, 1, 135, 240)), dtype=torch.float32, device=device))
            print(f"{output=}")
            output = output.cpu().detach().numpy()
            action = output[0,0]
            est_rew = output[0,1]
            print(f"cycle={total_loops:04}\t{distance_from_center=:.1f}\t{action=}\test_rew={est_rew}")
        ...
        if kph > 30:
            ...
            states.append(state.reshape(1, 135, 240))
...
def main():
    ...
    PATH = f"{newdir}/ResNet50-v7-actorcritic-continuousaction"
