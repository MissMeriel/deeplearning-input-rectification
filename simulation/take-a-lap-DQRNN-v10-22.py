...
def main():
    ...
    newdir = f"RLtrain-v1022badmodel-{timestr}-{randstr}"
    if not os.path.exists(newdir):
        os.mkdir(newdir)
        shutil.copy(f"{__file__}", newdir)
        shutil.copy(f"{os.getcwd()}/beamng_env.py", newdir)
    PATH = f"{newdir}/SB3-v1022badmodel-DQN6action-rew5000"
    import gym
    ...
    env = CarEnv(image_shape=(1, 84, 150), model="DQN", filepathroot=PATH, beamngpath='C:/Users/Meriel/Documents',
                 ...
                 beamnginstance="BeamNG.researchINSTANCE4", port=64956, road_id="10673", reverse=False)
    ...
    model = DQN(
        ...
        device="cuda",
        tensorboard_log=f"./{newdir}/tb_logs_v1022-badmodel/",
    )
    ...
    model.learn(
        total_timesteps=5e5, tb_log_name="dqn_v1022badmodel_sb3_car_run_" + str(time.time()), **kwargs
    )

    # Save policy weights
    model.save(f"{PATH}-dqn_v1022badmodel_sb3_car_policy.pt")
