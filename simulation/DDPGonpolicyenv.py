...
class CarEnv(gym.Env):
    ...
    def step(self, action):
        if self.test_model:
            ...
            if evaluation == 1:
                ...
                # vec3(601.547,53.4482,43.29) quat(-0.033713240176439,0.038085918873549,0.99870544672012,-0.00058328913291916)
                # vec3(491.521,119.336,30.25) quat(-0.062928266823292,0.038666535168886,0.68600910902023,0.72383457422256)
            ...
            outside_track, distance_from_center, leftrightcenter, segment_shape, theta_deg = self.has_car_left_track()
            # evaluation = self.evaluator(outside_track, distance_from_center, leftrightcenter, segment_shape)
            reward = self.calc_reward_from_expert(action, taken_action, expert_action)
            self.episode_steps += 1
    ...
    def calc_reward_from_centerline(self, dist_from_center):
        if self.state["collision"]:
            return 10000
        else:
            return math.pow(2, 4-dist_from_center)


    def calc_reward_from_expert(self, agent_action, car_action, expert_action):
        if self.state["collision"]:
            return -5000
        elif round(abs(car_action + agent_action.item() - expert_action), 3) < 0.01:
            return 10
        else:
            return (abs(car_action + agent_action.item() - expert_action)*10)
